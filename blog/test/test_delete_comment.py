...
class TestDeleteComment(GraphQLTestCase):
    def setUp(self):
    ...
    def test_can_delete_one_comment_from_one_user(self):
        ...
        deleteCommentResponse = self.query(
            DELETE_COMMENT_MUTATION,
            headers = {'HTTP_AUTHORIZATION': "JWT " + self.token},
            variables = {"id": getCommentsResponse['data']['commentByPostId'][0]['id']}
        )

        self.assertEqual(deleteCommentResponse.status_code, 200)
        self.assertResponseNoErrors(deleteCommentResponse)
        deleteCommentResponse = json.loads(deleteCommentResponse.content)
        assert len(deleteCommentResponse['data']['deleteComment']) == 1
        assert len(deleteCommentResponse['data']['deleteComment']['comment']) == 2
        ...
        getCommentsResponse = json.loads(getCommentsResponse.content)
        assert len(getCommentsResponse['data']['commentByPostId']) == 1
        assert getCommentsResponse['data']['commentByPostId'][0]['content'] \
                == "test post1 comment1 content"

    def test_can_delete_all_comments_from_one_user(self):
        getCommentsResponse = self.query(
            GET_COMMENTS_QUERY,
            variables = {"postId": self.createPostResponse['data']['createPost']['post']['id']}
        )
        getCommentsResponse = json.loads(getCommentsResponse.content)
        deleteCommentResponse1 = self.query(
            DELETE_COMMENT_MUTATION,
            headers = {'HTTP_AUTHORIZATION': "JWT " + self.token},
            variables = {"id": getCommentsResponse['data']['commentByPostId'][0]['id']}
        )

        deleteCommentResponse2 = self.query(
            DELETE_COMMENT_MUTATION,
            headers = {'HTTP_AUTHORIZATION': "JWT " + self.token},
            variables = {"id": getCommentsResponse['data']['commentByPostId'][1]['id']}
        )
    ...
    def test_can_delete_one_comment_while_comments_from_different_users(self):
        ...
        deleteCommentResponse1 = self.query(
            DELETE_COMMENT_MUTATION,
            headers = {'HTTP_AUTHORIZATION': "JWT " + self.token},
            variables = {"id": getCommentsResponse['data']['commentByPostId'][2]['id']}
        )
        ...
        self.assertResponseNoErrors(getCommentsResponse)
        getCommentsResponse = json.loads(getCommentsResponse.content)
        assert len(getCommentsResponse['data']['commentByPostId']) == 3
        assert getCommentsResponse['data']['commentByPostId'][0]['content'] \
    ...
    def test_cant_delete_comment_with_non_existing_id(self):
        ...
        self.assertEqual(deleteCommentResponse.status_code, 200)
        deleteCommentResponse = json.loads(deleteCommentResponse.content)
        assert deleteCommentResponse['errors'][0]['message'] == "Comment matching query does not exist."

    def test_cant_delete_comment_without_token(self):
        logging.disable(logging.ERROR)
        deleteCommentResponse = self.query(
            DELETE_COMMENT_MUTATION,
            variables = {"id": '9999'}
        ...
        self.assertEqual(deleteCommentResponse.status_code, 200)
        deleteCommentResponse = json.loads(deleteCommentResponse.content)
        assert deleteCommentResponse['errors'][0]['message'] == "Authentication credentials were not provided"